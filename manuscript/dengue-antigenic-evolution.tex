...
This suggests that the 12 phenotypes we report may be sufficiently distinct to have important impacts on secondary case outcome and vaccine efficacy.
Recent analysis of a longitudinal cohort study shows that neutralization titers correlate with protection from severe secondary infection; however, it is important to note that antibody neutralization and escape is only one component of the immune response to DENV.
\tbc{Is there another example here to cite? I'd ideally like 3 examples rather than just 2.}
...
\tbc{Paragraph on how our estimate of 12 phenotypes is a lower bound due to the structure of the titer data. I don't like labeling this paragraph `limitations'.}

\tbc{Discuss inability to tie individual mutations to antigenic impact due to too many mutations per branch and sparse data.}

\tbc{Discuss necessity of more detailed titer dataset to elucidate these issues.}

\tbc{Maybe also discuss deficiencies of the PRNT assay here, ie better assays also needed.}

\textbf{Limitations of the titer model}
We expect that the reported antigenic phenotypes represent a lower-bound on the true extent of intraserotype antigenic variation.
Our current titer dataset spans the breadth of DENV diversity, but provides limited resolution to detect within-genotype antigenic variation or to link individual mutations to antigenic change.
We have provided all code, configuration files and datasets at `github.com/blab/dengue-antigenic-dynamics` to encourage future studies' use of this model as a mechanistic framework to further investigate the genetic drivers and full extent of DENV antigenic heterogeneity as larger titer datasets become available.
\tbc{I'd make a separate single `methods sharing' paragraph towards the end of the Discussion. This methods sharing should cover both antigenic phenotypes and clade dynamics. I didn't like the repetition in current Discussion. I'd basically get rid of the paragraph.}
...
\tbc{You should describe in friendly language how the model works in this paragraph. Don't hand wave it aside with `antigenic novelty'. Avoid math here.}
...
\tbc{Paragraph describing how well the model works for serotype level data. Bridge this into a description of why the comparison of model fits for two genotype models is the right comparison. Discuss how well the model works for both genotype models.}
...
\tbc{All the above should be expanded. Cover better space and what metapopulation we are projecting. Cover the lack of outside imports.}
...
\tbc{Good paragraph, but cover in more detail. Say we really want real prevalence.}

\tbc{Model sharing section...}
...
\tbc{Conclusion: I'd suggest to relate to previous epi dynamics models and say that incorporating genetic and serological data has bright future for understanding dengue dynamics.}
