...
Unlike most infectious diseases, secondary DENV infections are more likely than primary infections to cause severe disease, with estimates of relative risk of severe dengue of $\sim$24. COMMMENT: WHERE IS THIS NUMBER FROM?
...
Thus, the antigenic relationships between dengue viruses --- whether the immune response generated after primary infection results in protection or enhancement of secondary infection --- are key drivers of DENV case outcomes and epidemic patterns.
...
For example, analysis of a longitudinal cohort study demonstrated that specific combinations of primary infection serotype and secondary infection genotype can mediate individual case outcomes. COMMENT: WHICH STUDY ARE YOU REFERING TO?
...
Normalized titers (COMMENT: MEANING ANTIGENIC DISTANCES?) between $i$ and $j$ are thus calculated as $T_{ij} = log_2(T_{ij}) - log_2(T_{ii})$ COMMENT: HOW DOES T_ij = T_ij+OTHER STUFF? COULD YOU USE OTHER NOTATION? ALSO, SHOULDN'T IT BE AUTOLOGOUS TITER - TEST TITER, LIKE IN FIG. 1? SORRY FOR THE CONFUSION IT JUST SEEMS BACKWARDS TO ME, such that a higher normalized titer value indicates that serum $i$ is less effective at neutralizing virus $j$, implying greater distance between viruses $i$ and $j$.
...
\begin{figure}[h]
  ...
  \includegraphics[width=0.6\textwidth]{../figures/png/genetic_antigenic_distance.png}
  	\caption{\textbf{Normalized antigenic distance vs. genetic distance between pairs of dengue strains.}  Normalized antigenic distance between virus strains from Figure 1 as a function of the patristic distance (COMMENT: CONSIDER DEFINING) between them on a maximum likelihood phylogeny of viral genomes (COMMENT: FULL GENOMES? E gene?).}
  	\label{genetic_antigenic_distance}
  \end{centering}
\end{figure}

Titer measurements are prone to noise, and there is a limited amount of available data. COMMENT: LIMITED DATA FOR WHAT?
...
The relationship between genetic distance and antigenic distance is consistent within and between serotypes, where the same trend line can be fit to both within and between comparisons (COMMENT: CONSIDER SHOWING BOTH?).
...
(COMMENT: I WOULD PUT NEAR WHERE YOU PUT THE TITER DATA, AND IN THE TEXT RIGHT WHEN YOU INTRODUCE THE IDEA OF PATRISTIC DISTANCE
\begin{figure}[h]
  \begin{centering}
...
Next, we infer how much antigenic change has occurred along each branch of the phylogeny. COMMENT: PERHAPS ADD BASED ON THE MEAUSURED ANTIGENIC DISTANCE BETWEEN SERUM X AND VIRUS Y AS MEAUSURED AS NEUTRALIZING ANTIBODY TITERS. MENTION IF USE ALL TITERS OR JUST THE NUMERIC?
This assigns each branch $b$ an antigenic distance $d_b$.
With this in hand, we estimate the antigenic distance between any pair of viruses (COMMENT: PERHAPS ADD INCLUDING THOSE NOT TITRATED) by tracing the path between them in the phylogeny, summing branch-specific distances $d_b$ as we go.
...
We take the training data and fit $d_b$ for each branch in the tree, subject to regularization. COMMENT: WHAT DO YOU MEAN BY REGULARIZATION? 
Parsimoniously, we expect that antigenic change is more likely to occur through fewer, larger changes than through many small changes; correspondingly, values of $d_b$ are exponentially distributed such that most values of $d_b = 0$. (COMMENT: BUT WHILE TRYING TO FIT THE ACTUAL TITER DATA, RIGHT? DOES IT MAKE SENSE TO DO THIS GIVEN THE DATA?)
This is analogous to lasso regression to identify a few parameters with positive weights and set other parameters to 0.
Additionally, some viruses have greater binding avidity, and some sera are more potent than others; these row and column effects, respectively, are normally distributed and are taken into account when estimating titers. (COMMENT: WHAT HAPPENS IF YOU DON"T NORMALIZE FOR THIS? DID YOU TRY IT? ALSO, DO YOU SHOW THE HEATMANT FOR THE TITERS ONCE YOU DO THE COL AND ROW ADJUSTMENT?)
...
This (COMMENT WHICH MODEL?) model is an effective tool for estimating antigenic relationships between viruses based on their relative positions in the phylogeny.
We can also use variations of this model to explicitly test whether the observed antigenic phenotypes are better explained by the hypothesis that dengue serotypes are antigenically uniform ('interserotype model'), or by the hypothesis that serotypes are antigenically diverse ('full tree model'). (COMMENT: NICE!)
...
The full tree model is able to predict test titers within $0.89$ $log_2$ titer units of the true value (RMSE approaching the level of error intrinsic to the assay), and explains $66\%$ of the observed variation in neutralization titers overall (Figure ~\ref{titer_model_performance}). (COMMENT: MAYBE THIS IS DUMB, BUT WHAT WOULD HAPPEN IF YOU ALLOW FOR WITHIN SEROTYPE VARIATION, BUT YOU JUST ASSIGNED TITERS RANDOMLY ACROSS BRANCHES? WOULD YOU GET AN OK FIT, OR WOULD IT BE AS BAD OR WORSE THAN THE NO INTRA-SEROTYPE VARIATION MODEL?)
Importantly, all reported error metrics refer to performance on test data, so this difference in model performance is not due to the number of free parameters. (COMMENT: NICE!)
...
We observe strong evidence that homotypic genotypes of dengue vary in their ability to escape antibody neutralization. (COMMENT: HOW?)
However, antibody neutralization is only one of many factors that contribute to secondary DENV infection outcomes.
We thus wanted to establish whether the observed antigenic diversity influences dengue population dynamics in the real world.
At any given time, many factors contribute to both the total size of the viral population (which corresponds to the incidence in the host population), and the makeup of the viral population (the relative frequency of each viral strain circulating).
For example, seasonal fluctuations in mosquito population size and other environmental factors influence the DENV population size.
...
It follows that these viruses have a growth rate--defined as the fold-change in frequency over time--greater than 1: $\frac{x_i(t+dt)}{x_i(t)} > 1$. COMMENT: DO YOU DO YOUR FIT ON A LOG SCALE? SO THAT DECLINE AND GROWTH CAN BE MEANINGFULLY COMPARED?
...
This also demonstrates that this model captures key components of dengue population dynamics, performing significantly better than the null model under which all clades have equal antigenic fitness (see Methods). (COMMENT: EQUAL ANTIGENIC FITNESS AT ALL TIMES? PERHAPS EXPLAIN MORE)
Interestingly, the model performs best when we model immunity as waning linearly over time, with the probability of protection dropping by about 48\% per year for the first two years after primary infection. (COMMENT:BEST AS COMPARED TO WHAT?)
...
Overall, we find that antigenic novelty contributes to genotype turnover, although it explains less of the observed variation (COMMENT: THAN WHAT?) (Figure ~\ref{genotype_growth_rates}A).
...
COMMENT: I REALLY LIKE THE IDEA OF THIS SECTION, BUT I FIND IT DIFFICULT TO UNDERSTAND. VALUES <1 growth ARE DECLINE, CORRECT? WHY NOT PLOT IT AND ANALYZE IT ON A LOG SCALE? AM I MISSING SOMETHING? ALSO, I FIND THE FIGURES SOMEWHAT CONFUSING. AND IS FIG. 6C SHOWING A GOOD FIT?  ALSO, I KNOW WE DISCUSSED THE NULL MODEL--IS THAT THE ONE ASSUMING EQUAL ANTIGENIC FITNESS? 

\section*{Discussion}

\subsection*{Within-serotype antigenic heterogeneity}
We show that mapping antigenic change to individual phylogenetic tree branches and interpolating across the DENV phylogeny is able to explain a large majority of the observed variation in antigenic phenotypes as measured by neutralization titers.
...
(COMMENT: I WOULD WAIT UNTIL A BIT FURTHER IN THE DISCUSSION. HAVE THE FIRST PART OF THE DISCUSSION SUMMARIZE WHAT YOU DID AND THE STRENGTHS OF YOUR STUDY)
We also find that accounting for within-serotype antigenic evolution (COMMMENT: ADD IN TAKING INTO ACCOUNT PHYLOGENETIC RELATIONSHIPS) is necessary to explain the observed variation in antigenic phenotypes.
...
We observe at least 12 distinct antigenic phenotypes that are both genetically and antigenically distinct. (COMMENT: COULD THEY BE ANTIGENICALLY BUT NOT GENETICALLY DISTINCT? IF NOT, MAKE SURE TO EXPLAIN THIS AS A STRENGTH OF YOUR STUDY)
On average, intraserotype antigenic phenotypes are 0.42 titer units apart and interserotype antigenic phenotypes are 1.78 titer units apart. (COMMENT: I WOULD MENTION THIS IN THE ACTUAL RESULTS, NOT HERE. ALSO, THIS SOUNDS LIKE A MAJOR WEAKNESS, UNLESS YOU MEAN FOR IT TO BE SO HERE, I WOULD PUT IN THE RESULTS OR IN A LIMITATIONS SECTION)
...
Our current titer dataset spans the breadth of DENV genetic diversity, but in most cases lacks the resolution to detect sub-genotype antigenic variation or to identify which specific mutations correspond to changes in antigenic phenotype due to small sample size.
The appearance of the deep antigenic divergence of the four serotypes, and the more recent antigenic divergences within each serotype, suggest that DENV antigenic evolution is likely an ongoing, though gradual, process. (COMMENT: BASED ON YOUR STUDY, COULD YOU PUT A TIME PREDICTION ON IT? :)
...
If we knew the true value of these censored titers, many of them would indicate larger antigenic distances than the reported values used to train the model. (COMMENT: EXPLAIN THEN IN THE RESULTS SECTION HOW THESE TITERS ARE MODELED, AND THEN DISCUSS THIS LIMITATION HERE)
...
Analysis of a longitudinal cohort study shows that these neutralization titers correlate with protection from severe secondary DENV infection, it is important to note that
DENV case outcomes are also mediated by interactions with innate and T-cell immunity, the effects of which are not captured in neutralization titers. (COMMENT: ALSO MENTION NOT SURE EXACTLY HOW MAPS TO ADE) % the fact that titer-based estimation of antigenic fitness is predictive of clade dynamics provides additional validation of neuts as a measure of antigenic distance 
...
This model places a fitness value on each serotype that derives from a constant intrinsic component alongside a time-dependent antigenic component. (IN THE RESULTS, DO YOU PLOT INTRISIC DIFFERENCEES? IT MIGHT BE NICE TO ALSO SHOW IF YOU SEE INTRINSIC DIFFERENCES, SUGGESTING DIFFERENTIAL INTRINSIC FITNESS FOR DIFFERENT GENOTYPES)
...
We find that DENV dynamics are most consistent with the hypothesis that immunity wanes linearly over a period of 2 years, %consistent with Leah's PNAS paper? (COMMENT: NO, WE DON"T REALLY SEE WANING PAST ABOUT 8 MONTHS, SORRY) SO IT DOES REQUIRE INDIVIDUAL WANING OF IMMUNITY?  
...
This suggests that although we find strong evidence that genotypes vary in their ability to escape neutralizing antibodies, these differences are subtle enough that they do not impact broad-scale regional dynamics over time. (COMMENT: HOW MUCH MORE AFFECTED BY INTRNISIC DIFFERENCES AMONG GENOTYPES? IS IT A TRADE OFF?)
...
% Distinguish between the size of the viral population (i.e., true population prevalence) and the composition of the viral population (i.e., viral clade frequency).
% The size of the viral population represents the epidemic magnitude, and is immensely important for public health policy. %epi models here
% couple of sentences about the promise of combining these two approaches using genetic and serological data

(COMMENT: YES, NICE POINTS)
...
% We encourage future studies' use of the titer model as a mechanistic framework to further investigate the genetic drivers and full extent of DENV antigenic heterogeneity as larger titer datasets become available.
% We also encourage the reuse and extension of the fitness model as a basis for understanding how population immunity impacts prevalence and epidemic magnitude.
