...
%%%%%%%% TO DO LIST %%%%%%%%%%
% - Add Leah's textual changes
% - Add titer symmetry figure to supplement
...
% - Update T_{ij} -> D_{ij} notation throughout (esp. methods)
...
\begin{figure}[h]
  ...
  \includegraphics[width=0.6\textwidth]{../figures/png/genetic_antigenic_distance.png}
  	\caption{\textbf{Normalized antigenic distance vs. genetic distance between pairs of dengue strains.} Antigenic distances are the same as from Figure ~\ref{titer_heatmap}; genetic distances are the patristic distances between viruses on a maximum likelihood phylogeny. \lkc{ FULL GENOMES? E gene?}.}
...
\begin{figure}[h]
  \begin{centering}
    ...
    \caption{\textbf{Tree of dengue antigenic phenotypes.}  A maximum likelihood phylogeny of DENV genomes was used to infer the topology, with branch lengths scaled to reflect $d_b$, the relative contributions of each branch to antigenic divergence, as inferred by the `full tree' model of DENV evolution. Antigenically uniform clades were collapsed, and node diameter reflects the size of the collapsed clade. \lkc{ COOL! I LIKE IT! SO CAN YOU INFER THA DENV2 AMERICAN IS CLOSER TO THE OTHER SEROTYPES? ALSO< IT SEEMS LIKE SOMEHTING IS STRANGE ABOUT THE LABELING?}}
...
It follows that these viruses have a growth rate--defined as the fold-change in frequency over time--greater than 1: $\frac{x_i(t+dt)}{x_i(t)} > 1$. \lkc{DO YOU DO YOUR FIT ON A LOG SCALE? SO THAT DECLINE AND GROWTH CAN BE MEANINGFULLY COMPARED?}
...
We also find that accounting for within-serotype antigenic evolution is necessary to explain the observed variation in antigenic phenotypes. \lkc{ADD IN TAKING INTO ACCOUNT PHYLOGENETIC RELATIONSHIPS}
...
This model places a fitness value on each serotype that derives from a constant intrinsic component alongside a time-dependent antigenic component.\lkc{IN THE RESULTS, DO YOU PLOT INTRISIC DIFFERENCEES? IT MIGHT BE NICE TO ALSO SHOW IF YOU SEE INTRINSIC DIFFERENCES, SUGGESTING DIFFERENTIAL INTRINSIC FITNESS FOR DIFFERENT GENOTYPES}
...
This suggests that although genotypes are antigenically diverse, these differences do not appear to influence large-scale regional dynamics over time. \lkc{ HOW MUCH MORE AFFECTED BY INTRNISIC DIFFERENCES AMONG GENOTYPES? IS IT A TRADE OFF?}
