...
% SB: shorten this explanation (move most to methods), give people intuition for how higher *normalized* titers correspond to *greater* distance instead
...
First, we build a phylogeny of dengue virus sequences to establish the genetic relationships between viruses (Figure ~\ref{genotype_tree}).
...
\tbc{We validate each model by using model parameters estimated from the training data to predict test data values...}
...
The full tree model performance is comparable to the model error from a cartography-based characterization of the same dataset \tbc{give this error from Leah's paper}, and to the error observed when this model was used to characterize an influenza dataset \tbc{also give this error, overly opaque without numbers}.
...
% SB: Move the rest of this paragraph to discussion, and collapse this subsection with the one above?
...
For context, analysis of the recent CYD-TDV vaccine trial shows different vaccine efficacy against genotypes I and II of DENV4, which are 0.4 log2 titer units apart in our dataset. \tbc{This should be in Discussion instead.}
...
\tbc{Spell this out more. This is a red flag for reviewers (for good reason).}
