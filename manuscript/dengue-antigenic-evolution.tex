...
We use these inferred antigenic relationships to directly quantify the extent to which antigenic fitness determines the composition of the DENV population in a real-world hyperendemic population.
We measure serotype frequencies across Southeast Asia through time and construct a model to estimate how they will fluctuate.
...
We find that antigenic fitness is able to explain most of the observed variation in serotype growth and decline in a hyperendemic population (Figure ~\ref{serotype_fitness_model}).
This demonstrates that antigenic fitness is a strong determinant of DENV serotype success and decline.

This model also allows us to examine how waning immunity and intrinsic viral fitness interact with DENV antigenic fitness.
...
We also use this model to quantify the extent to which within-serotype antigenic differences determine the success and decline of canonical DENV genotypes.
As for serotypes, we estimate the frequencies and antigenic fitness of each canonical genotype over time based on available sequence data and the frequency-weighted antigenic distance from recently circulating viruses, respectively.
We can calculate antigenic fitness based on either serotype-level (`interserotype model') or genotype-level (`full tree') antigenic distances.
If genotype-level antigenic variation impacts regional DENV population dynamics, then incorporating these specific differences into our estimates of antigenic fitness should better explain the observed fluctuations in genotype frequency over time, compared to estimates of antigenic fitness based on the coarser serotype-level antigenic relationships between genotypes.
We find that antigenic fitness explains a large portion of the observed variation in genotype growth and decline, but incorporating within-serotype antigenic differences does not improve our predictions (Figure ~\ref{genotype_growth_rates}).
...
However, this observation is limited by the available data and model assumptions.
