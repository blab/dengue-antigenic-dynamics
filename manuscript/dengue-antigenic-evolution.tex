...
%%%%%%%% TO DO LIST %%%%%%%%%%
% - Add references
% - One last audit of language use for "clade" vs "genotype" vs "serotype" vs "strain"

%%%%%%% TO DISCUSS %%%%%%%%%%%%
% - Perhaps redesign the serotype trajectory figure (panel C) to better illustrate the rolling immunity calculation
% - How to adjust / display the antigenic tree? potential for misleading interpretation in terms of which clades are more closely related antigenically based on the genetic topology. Also, fix labels.
...
% - Refit intrinsic fitness values such that DENV4_f0 = 1 and all others are > 1? This would allow direct comparison between these parameter values, as well as discussion of proportion of fitness attributable (antigenic vs intrinsic) & possible "tradeoffs"? Or leave this well enough alone -- might be overinterpreting
...
Although each serotype has been assumed to antigenically uniform, antigenic cartography suggests that antigenic heterogeneity may exist within each serotype, but its source, extent and impact is unclear.
Here, we construct a phylogenetics-based model to directly map antigenic change to underlying genetic divergence.
...
We find that antigenic fitness drives DENV population turnover, although this appears to be driven by coarser serotype-level antigenic differences.
...
Recent efforts to antigenically characterize diverse DENV strains suggests that each serotype may contain antigenic heterogeneity, but the source and impact of this heterogeneity is not clear.
...
To measure the pairwise antigenic distances for a panel of diverse DENV strains (Figure ~\ref{titered_strains_tree}), Katzelnick et al. infected naive non-human primates (NHP) with each virus, drew sera at three months post-infection, and then titered this sera against a panel of test viruses.
...
\begin{figure}[h]
\begin{centering}
\includegraphics[width=0.4\textwidth]{../figures/png/titer_heatmap.png}
    \caption{\textbf{Normalized antigenic distance between pairs of dengue virus strains and sera.}
    Aggregated neutralization titers from Katzelnick et al. were normalized such that the distance between autologous virus-serum pairs is 0, and each titer unit corresponds to one 2-fold change in PRNT50 value.
...
\begin{figure}[h]
  \begin{centering}
  \includegraphics[width=0.4\textwidth]{../figures/png/genetic_antigenic_distance.png}
...
With this in hand, we interpolate the antigenic distance between pairs of viruses by tracing the path between them in the phylogeny, summing branch-specific distances $d_b$ as we go (Methods, Eq. ~\ref{eq_predicted_titers}).
...
We take the training data and fit $d_b$ for each branch in the tree, subject to regularization as defined by the following constraints (Methods, Eq. ~\ref{eq_cost_fn}).
...
\sbc{I think this subsection is a bit short, but most of the obvious expansions are (appropriately, I think) in the discussion. Thoughts? I do like having a separate subsection explicity defining the 12 phenotypes, though.}
...
\begin{figure}[h]
  \begin{centering}
    ...
    \sbc{I'm not sure I actually like this figure, but I can't think of a better representation. See comment for discussion (top) re: possible misinterpretation.}
    }
...
We estimated the relative population frequency of each DENV serotype at three month intervals, $x_i(t)$ (Figure ~\ref{serotype_fitness_model}A), based on available sequence data (Methods, Eq. ~\ref{eq_empirical_frequency}).

Fitter viruses increase in frequency over time, such that $x_i(t+dt) > x_i(t)$.
...
% \sbc{I feel like this is maybe overinterpreting and likely to confuse people, so I took it out for now. Thoughts?}
% This also demonstrates that this model captures key components of dengue population dynamics; examining the optimal formulation of this model in more detail can yield insights into how antigenic relationships influence DENV population composition.
...
% We also find that these dynamics are best explained by intrinsic fitness that moderately varies by by serotype (Table ~\ref{parameter_values}). \sbc{Put a more detailed discussion of intrinsic fitness values between serotypes and the relative contribution of fa vs f0 here}
...
When antigenic distance is estimated from the `interserotype model', we find that antigenic fitness \sbc{fix wording -- it's total fitness?} explains approximately 31\% of the observed variation in genotype growth rates, and correctly predicts genotype growth vs. decline 68\% of the time (Figure ~\ref{genotype_fitness}C).
...
\includegraphics[width=\linewidth]{../figures/png/genotype-fitness.png}
    ...
    \textbf{C, D}  Antigenic fitness \sbc{again, fix wording -- it's total fitness?}, derived from antigenic distances was used to predict genotype growth or decline over a 5-year period.
...
We compute normalized antigenic distance between virus $i$ and serum $j$ (denoted $D_{ij}$) from measured titers relative to autologous titers (denoted $T_{ii}$ and $T_{ij}$, respectively), such that
\begin{equation}
  \label{eq_titer_norm}
D_{ij} = log_2(T_{ii}) - log_2(T_{ij})
\end{equation}
