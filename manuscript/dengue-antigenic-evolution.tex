...
Thus, we expect that our model systematically underestimates titer distances overall.
DENV4 genotypes I and II are estimated to be 0.4 log2 titer units apart; these strains were recently shown to be sufficiently antigenically distinct to differentiate secondary case outcomes in vivo.
...
If so, we would expect that as a given virus $i$ circulates in a population, the proportion of the population that is susceptible to infection with $i$--and other strains antigenically similar to $i$--decreases over time as more people acquire immunity.
Thus, if antigenic novelty constitutes a fitness advantage, then we would expect antigenic distance from recently circulating strains to correlate with high growth rates.

To test this hypothesis, we estimate the antigenic fitness of virus $i$ as a function of the antigenic distance from each strain $j$ that has circulated in a given population over the past 2 years, weighted by the relative frequency of $j$ and adjusted for linearly waning immunity.
We then predict their relative growth rate over the next 5 years (at time $t = t_0 + 5$) as a function of initial frequency (at time $t=t_0$), serotype-specific intrinsic fitness, and antigenic fitness (with accumulated immunity compounded every 3 months). % this needs a ton more explanation and clarification, but it's a start.

We applied this model to dengue dynamics in Southeast Asia from 1970 to 2015, with relative clade frequencies estimated from available sequence data (see methods).

This model predicts serotype growth vs decline correctly for 79\% of predictions.
Antigenic novelty explains 59\% of the observed variation in serotype growth rates.
