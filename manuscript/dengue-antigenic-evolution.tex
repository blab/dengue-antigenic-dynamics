...
To do so, we adapted a phylogeny-based model originally developed for influenza*.
...
\sbc{I think this subsection is a bit short, but most of the obvious expansions are (appropriately, I think) in the discussion. Thoughts? I do like having a separate subsection explicitly defining the 12 phenotypes, though.}
\tbc{Leave paragraph as it is and where it is, but remove subsection heading `There are at least...'}
...
\end{equation}
Where $D_{ij}$ is the antigenic distance between $i$ and each non-overlapping clade $j$, $n$ is the number of years since exposure, and $x_j(t-n)$ is the relative frequency of $j$ at year $t$ \tbc{check this --- sentence term describe both $t$ and $n$}.
...
\end{equation}
for short-term predictions (where $dt < 1$ year). \tbc{I believe the denominator needs to have the exponential term as well.}
...
\centering
  ...
  \tbc{I find it confusing when later results are introduced in beginning figures. At this point in the reading I don't have any idea what you mean by `full tree' model. I suggest to make this is a simple ML tree. Remove the titer model coloring.}
	}
...
\begin{figure}[h]
  ...
  \tbc{This would be better with a wider aspect ratio.}
  }
