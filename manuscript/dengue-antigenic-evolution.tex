...
For example, analysis of a longitudinal cohort study demonstrated that specific combinations of primary infection serotype and secondary infection genotype can mediate individual case outcomes. \lkc{ WHICH STUDY ARE YOU REFERING TO?}
...
Normalized titers \lkc{ MEANING ANTIGENIC DISTANCES?} between $i$ and $j$ are thus calculated as $T_{ij} = log_2(T_{ij}) - log_2(T_{ii})$, such that a higher normalized titer value indicates that serum $i$ is less effective at neutralizing virus $j$, implying greater distance between viruses $i$ and $j$.
\lkc{ HOW DOES Tij = Tij+OTHER STUFF? COULD YOU USE OTHER NOTATION? ALSO, SHOULDN'T IT BE AUTOLOGOUS TITER - TEST TITER, LIKE IN FIG. 1? SORRY FOR THE CONFUSION IT JUST SEEMS BACKWARDS TO ME}
...
\begin{figure}[h]
  ...
  \includegraphics[width=0.6\textwidth]{../figures/png/genetic_antigenic_distance.png}
  	\caption{\textbf{Normalized antigenic distance vs. genetic distance between pairs of dengue strains.}  Normalized antigenic distance between virus strains from Figure 1 as a function of the patristic distance \lkc{ CONSIDER DEFINING} between them on a maximum likelihood phylogeny of viral genomes \lkc{ FULL GENOMES? E gene?}.}
  	\label{genetic_antigenic_distance}
  \end{centering}
\end{figure}

Titer measurements are prone to noise, and there is a limited amount of available data. \lkc{ LIMITED DATA FOR WHAT?}
...
The relationship between genetic distance and antigenic distance is consistent within and between serotypes, where the same trend line can be fit to both within and between comparisons \lkc{ CONSIDER SHOWING BOTH? }.
...
\lkc{ NOT SURE WHAT THIS MEANS}

% SB: This is a weird place for this figure to go, but I can't quite decide where to put it. Thoughts?
\lkc{ I WOULD PUT NEAR WHERE YOU PUT THE TITER DATA, AND IN THE TEXT RIGHT WHEN YOU INTRODUCE THE IDEA OF PATRISTIC DISTANCE}
\begin{figure}[h]
  \begin{centering}
...
Next, we infer how much antigenic change has occurred along each branch of the phylogeny. \lkc{ PERHAPS ADD BASED ON THE MEAUSURED ANTIGENIC DISTANCE BETWEEN SERUM X AND VIRUS Y AS MEAUSURED AS NEUTRALIZING ANTIBODY TITERS. MENTION IF USE ALL TITERS OR JUST THE NUMERIC?}
This assigns each branch $b$ an antigenic distance $d_b$.
With this in hand, we estimate the antigenic distance between any pair of viruses \lkc{ PERHAPS ADD INCLUDING THOSE NOT TITRATED} by tracing the path between them in the phylogeny, summing branch-specific distances $d_b$ as we go.
...
This \lkc{ WHICH MODEL?} model is an effective tool for estimating antigenic relationships between viruses based on their relative positions in the phylogeny.
We can also use variations of this model to explicitly test whether the observed antigenic phenotypes are better explained by the hypothesis that dengue serotypes are antigenically uniform ('interserotype model'), or by the hypothesis that serotypes are antigenically diverse ('full tree model'). \lkc{ NICE!}
...
We observe strong evidence that homotypic genotypes of dengue vary in their ability to escape antibody neutralization. \lkc{ HOW?}
...
This also demonstrates that this model captures key components of dengue population dynamics, performing significantly better than the null model under which all clades have equal antigenic fitness (see Methods). \lkc{ EQUAL ANTIGENIC FITNESS AT ALL TIMES? PERHAPS EXPLAIN MORE}
Interestingly, the model performs best when we model immunity as waning linearly over time, with the probability of protection dropping by about 48\% per year for the first two years after primary infection. \lkc{BEST AS COMPARED TO WHAT?}
...
Overall, we find that antigenic novelty contributes to genotype turnover, although it explains less of the observed variation \lkc{ THAN WHAT?} (Figure ~\ref{genotype_growth_rates}A).
...
\lkc{ I WOULD WAIT UNTIL A BIT FURTHER IN THE DISCUSSION. HAVE THE FIRST PART OF THE DISCUSSION SUMMARIZE WHAT YOU DID AND THE STRENGTHS OF YOUR STUDY)}
...
We observe at least 12 distinct antigenic phenotypes that are both genetically and antigenically distinct. \lkc{ COULD THEY BE ANTIGENICALLY BUT NOT GENETICALLY DISTINCT? IF NOT, MAKE SURE TO EXPLAIN THIS AS A STRENGTH OF YOUR STUDY}
On average, intraserotype antigenic phenotypes are 0.42 titer units apart and interserotype antigenic phenotypes are 1.78 titer units apart. \lkc{ I WOULD MENTION THIS IN THE ACTUAL RESULTS, NOT HERE. ALSO, THIS SOUNDS LIKE A MAJOR WEAKNESS, UNLESS YOU MEAN FOR IT TO BE SO HERE, I WOULD PUT IN THE RESULTS OR IN A LIMITATIONS SECTION}
...
The appearance of the deep antigenic divergence of the four serotypes, and the more recent antigenic divergences within each serotype, suggest that DENV antigenic evolution is likely an ongoing, though gradual, process. \lkc{ BASED ON YOUR STUDY, COULD YOU PUT A TIME PREDICTION ON IT? :)}
...
If we knew the true value of these censored titers, many of them would indicate larger antigenic distances than the reported values used to train the model. \lkc{ EXPLAIN THEN IN THE RESULTS SECTION HOW THESE TITERS ARE MODELED, AND THEN DISCUSS THIS LIMITATION HERE}
...
We find that DENV dynamics are most consistent with the hypothesis that immunity wanes linearly over a period of 2 years, %consistent with Leah's PNAS paper? \lkc{ NO, WE DON"T REALLY SEE WANING PAST ABOUT 8 MONTHS, SORRY) SO IT DOES REQUIRE INDIVIDUAL WANING OF IMMUNITY?}
...
This suggests that although we find strong evidence that genotypes vary in their ability to escape neutralizing antibodies, these differences are subtle enough that they do not impact broad-scale regional dynamics over time. \lkc{ HOW MUCH MORE AFFECTED BY INTRNISIC DIFFERENCES AMONG GENOTYPES? IS IT A TRADE OFF?}
