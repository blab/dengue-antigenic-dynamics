...
\sbc{N.B. Trevor is still editing this paragraph}
Given the relatively long timescale of dengue evolution, we expect that dengue antigenic diversity has been shaped by many instances of parallelism, reversion and homoplasy.
Indicative of this, our model identifies four pairs of reversions where both the forward and reverse substitutions are inferred to be antigenically relevant (Table~\ref{antigenic_mutations}).
To investigate the impact of these reversions and parallelisms, we compared this `substitution' model to a similar model formulation which assigns $d_m$ values to individual branches in the phylogeny, rather than to individual mutations, and estimates symmetrical titer distances as the sum of $d_m$ values along the shortest path separating two viruses in the phylogeny \citep{neher2016prediction}.
By definition, branches in the phylogeny directly capture the effect of all mutations that occurred along each branch, except for reversions and parallelism.
