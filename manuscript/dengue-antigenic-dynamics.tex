...
While previous studies have identified epitopes on both prM and E, it is believed that antibodies involved in ADE primarily target prM, while neutralizing antibodies primarily target E. %ADD REFERENCE
...
\begin{figure}[ht]
  ...
  \includegraphics[width=\textwidth]{../figures/png/mutations_position_db.png}
    ...
    % The y axis indicates antigenic effect size (note that \textbf{A} has an independent y scale for visual clarity); the x axis indicates mutations' position in \textit{E}, relative to each functional domain as noted in \textbf{B}.}
...
Pairwise comparisons of all serum strains and test viruses yields 1534 unique mutations that are observed at least twice.
Our parsimonious model attributes antigenic change to a total of 49 specific mutations and four colinear mutation clusters (consisting of 2---6 co-occurring mutations).
...
\begin{figure}[ht]
  \begin{centering}
    \includegraphics[width=\textwidth]{../figures/png/vaccine_titer_trees.png}
        ...
        Here, we show the estimated antigenic distance between each test virus in the tree and serum raised against each monovalent component of the NIH vaccine candidate (indicated as `X').
        }
...
As shown in Figure~\ref{vaccine_titer_trees}, vaccine-elicited antibodies result in strong homotypic neutralization, but heterotypic cross-neutralization is varies widely between specific strains.
This has important ramifications of vaccine design and trial evaluation.
...
In summary, we have identified a small number of relevant mutations that explain observed dengue antigenic heterogeneity.
...
% introduce new seasia dataset here
...
% add note about thai comparisons here
...
% reference the simulation validation here
...
% reference table of error metrics here
...
% add note about null model performance here.
...
In total, our dataset includes 1182 measurements across 48 virus strains in total: 36 of these were used to generate serum, and 47 were used as test viruses.
...
We selected all remaining virus strains that were annotated as a Southeast Asian isolate (total N = 8644).
...
% move tables of parameter values and performance metrics here
...
% Add table of error metrics here
...
% add species titer error figure here
...
% add thai case counts comparison here


% add simulation figure(s) here
