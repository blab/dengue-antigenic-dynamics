...
While previous studies have identified epitopes on both prM and E, it is believed that antibodies involved in ADE primarily target prM, while neutralizing antibodies primarily target E. \sbc{Leah, do you remember this reference?} %ADD REFERENCE
...
\begin{figure}[ht]
  \begin{centering}
    ...
    \textbf{C} illustrates DENV1 frequencies between 1983 and 1987 as an example of how the model predicts clade growth rates.
...
This simple model explains 48\% of the observed variation in serotype growth rates, and predicts serotype growth vs. decline correctly for 66\% of predictions (Figure~\ref{serotype_fitness_model}D).
...
We find that serotype fluctuations are most consistent with a model wherein population immunity wanes linearly over time, with the probability of protection dropping by about 16\% per year for the first two years after primary infection.
...
These mutations occur both between and within serotypes, resulting in within-serotype antigenic variation.
...
To investigate this in more detail, we examine patterns of neutralization in response to vaccination with each monovalent component of the NIH vaccine candidate.
...
This is consistent with previous reports of secondary infection genotype as an



Using these mutations to inte
...
%%%% PUNT ALL OF THIS TO THE DISCUSSION
% something about how this kind of effect modification of heterotypic cross-protection could impact ADE, fits with previous observations

% This set of identified antigenic mutations includes four pairs of reversions, where both the forward and reverse mutations at the same site confer antigenic change: L122S, I129V, I162A, and N203E.
...
% This model also % does an OK job, but does not perform as well.
% Branches and mutations generally correspond to one another, with the exception of reversions.
...
% Add a paragraph here about reversions and push/pull? Domains of most common mutations within/between serotypes?
...
\subsubsection{Null model and model performance}\\
...
We assess predictive power as the sum of squared error between predicted and empirical clade frequencies.
As above, we divide the interval between $t$ and $dt$ into a total of $U$ 3 month timepoints, $[t+u, t+2u, ... t+U]$, such that $t+U=dt$.
We sum the squared prediction error at each of these intermediate timepoints for each prediction to assess both the final frequeny predictions and the predicted clade trajectories:

\begin{equation}
  \label{eq_sse}
SSE = \sum_{i,t} \left( \sum_{u} \hat{x_i}(t+u) - x_{i}(t+u) \right)^2
\end{equation}

We can then estimate how much more error is present in the `equal null' model than the antigenically-informed model:
\begin{equation}
  \label{eq_delta_sse}
\Delta SSE = SSE^{equal_null} - SSE^{model}
\end{equation}

Our frequency prediction model has a total of 8 free parameters.
For each dataset, we jointly fit these parameters to maximize $\Delta SSE$ via a sweep through parameter space (Table~\ref{fitness_model_parameters}).
...
\begin{centering}
  \begin{table}[ht]
    \resizebox{\textwidth}{!}{
    \begin{tabular}{llrr}
Mutation & Serotype & Effect size $d_b$ & Count \\
\hline
...
Each row represents a mutation (or colinear cluster of mutations) inferred by the titer model to have a non-zero antigenic effect size $d_b$.
...
% add table of antigenic mutations here
...
\begin{centering}
  \begin{table}[ht]
    ...
    \begin{tabular}[ht]{ l | l | r | r }
      \hline
      Genetic resolution & Fitness model & $\Delta$ SSE & Pearson $R^2$ & Accuracy & Mean absolute error\\
      \hline
      Serotype & Interserotype & 3.67 & 0.48 & 0.66 & 0.11 \\
      Genotype & Full tree & 2.56 & 0.22 & 0.62 & 0.10 \\
      Gentoype & Interserotype & 2.49 & 0.26 & 0.66 & 0.10 \\ \hline

      Serotype & Antigenic null & -1.06 & 0.03 & 0.52 & 0.13 \\
      Serotype & Equal null & 0.0 & 0.0 & 0.47 & 0.13\\     \hline

      Genotype & Antigenic null & -0.7 & 0.03 & 0.52 & 0.11 \\
      Genotype & Equal null & 0.0 & 0.0 & 0.43 & 0.11 \\     \hline
    \end{tabular}
    }
  \label{fitness_model_performance}
  \caption{
  \textbf{Fitness model performance comparisons}
  Here we compare the performance of the `antigenically informed' fitness model to model performance under two null formulations.
