...
For brevity, these normalized titer values are hereafter referred to simply as $log_2(titers)$.
...
Our parsimonious model attributes antigenic change to a total of 49 specific mutations and 4 colinear mutation clusters (each consisting of 2---6 co-occurring mutations) (Figures~\ref{mutations_db},~\ref{mutation_positions},  Table~\ref{antigenic_mutations}).
Each of these mutations confers 0.01---2.11 (median 0.19) log$_2$ titer units of antigenic change (Figure~\ref{mutations_db}).
As seen in Figure~\ref{mutation_positions}, these mutations span all major domains of \textit{E}, and most occur both between and within serotypes.
...
This demonstrates that although intrinsic fitness plays an important role in dictating long term dynamics, antigenic fitness is a substantial determinant of DENV serotype dynamics in a real-world, hyperendemic population.
...
\end{equation}
We used $\lambda = 3.0$, $\gamma = 0.6$, and $\delta = 1.2$ to minimize test error. \sbc{@trvrb - is there a different standard notation to replace $\gamma$ here? It's also used in the fitness model notation.}
...
We jointly fit these parameters to minimize RMSE of serotype frequency predictions via the Nelder-Mead algorithm as implemented in SciPy v.1.0.0 (Table~\ref{fitness_model_parameters}) \citep{Jones:2001aa,gao2012implementing}.
...
\begin{table}[ht]
  ...
  \begin{tabular}{ c c c c c }
    ...
    Gentoype            & Interserotype  & 0.062  & 0.286         & 0.666 \\
