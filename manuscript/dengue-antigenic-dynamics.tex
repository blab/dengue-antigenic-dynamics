...
On average across cross-validation replicates, this model yields a root mean squared error (RMSE) of 0.75 when predicting titers relative to their true value (95\% CI 0.74--0.77, RMSE), \tbc{the sentence that this model predicts within 0.75 implies absolute error, but the parethentical says RMSE, I've revised with the assumption that this is indeed RMSE} and explains 78\% of the observed variation in neutralization titers overall (95\% CI 0.77--0.79, Pearson $R^2$).
...
This comparison model, termed the `tree model', assigns $d_m$ values to individual branches in the phylogeny, rather than to individual mutations, and estimates symmetrical titer distances as the sum of $d_m$ values along the shortest path separating two viruses in the phylogeny \citep{neher2016prediction}.
...
This tree model also explains observed antigenic phenotypes to a reasonable degree, but has higher error and less predictive power than the asymmetrical substitution model presented here (Table ~\ref{titer_model_performance}).
...
\tbc{I'd reformulate this paragraph. Good to discuss substitution-model vs. tree-model. But I wouldn't frame it as `there are 4 mutations that have reversions'. There are a whole mess of mutations that are parallelisms or reversions across the tree. These will cause the two models to perform differently. Start with this. Mention substitution model outperforms tree model. Then, I'd mention these reversions and the fact that these bring some distant genotypes slightly more closely together (as seen in Figure \ref{genotype_dTiter_heatmap}).}
...
To predict unmeasured titers, we employ the `substitution model' from \citet{neher2016prediction} and implemented in Nextstrain \citep{hadfield2017nextstrain}, which assumes that antigenic evolution is driven by underlying genetic evolution.
...
\end{equation}
where $d_m$ is the titer drop assigned to each mutation, $m$, between serum $i$ and virus $j$, and $b$ iterates over mutations.
...
We gratefully acknowledge the authors \citep{katzelnick2015dengue} and developers \cite{andersen2013cvxopt,cock2009biopython,gao2012implementing,hadfield2017nextstrain,Hunter2007computing,Jones2001scipy,katoh2013mafft,mckinney2010data,nguyen2014iq,perez2007ipython,seabold2010statsmodels,van2011numpy} who make our work possible.
\tbc{I've never seen citations in acknowledgements. You should spell these out if you want to include them. But citing software is good and I'd move these as citations to Methods somewhere. Perhaps in Data Availability.}
...
\begin{table}[ht]
  ...
  \begin{tabular}{ c c c c c }
    ...
    Genotoype           & Interserotype  & 0.062  & 0.286         & 0.666 \\
