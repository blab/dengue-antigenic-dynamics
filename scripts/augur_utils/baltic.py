...
def loadJSON(tree_path,absoluteTime=True,verbose=False,sort=True,stats=True,name_attr='strain'):
    ll=tree()
    with open(tree_path) as json_data:
        d = json.load(json_data)
        make_treeJSON(ll,'root',d,name_attr=name_attr)

    if absoluteTime==True:
        for k in ll.Objects:
            try:
                k.absoluteTime=float(k.traits['num_date'])
            except:
                print type(k)
                print k.traits
                raise KeyError
            k.height=float(k.traits['tvalue'])

    else:
        for k in ll.Objects:
            k.height=float(k.traits['xvalue'])
...
def make_treeJSON(tree,parent,JSONnode,name_attr='strain'):
    if 'attr' in JSONnode:
    ...
    if JSONnode.has_key('children'): ## only nodes have children
        ...
        new_node=leaf()
        new_node.numName=JSONnode[name_attr] ## set leaf numName

    new_node.parent=tree.cur_node ## set parent-child relationships
    tree.cur_node.children.append(new_node)
    new_node.index=JSONnode[name_attr] ## indexing is based on name
    ...
    if JSONnode.has_key('children'):
        for child in JSONnode['children']:
            make_treeJSON(tree,JSONnode,child,name_attr)
...
class node: ## node class
    ...
    def rotate(self):
    	...
    	curr_children.reverse()
    	self.children = curr_children
...
# >>>>>>>
    ...
    def commonAncestor(self,descendants,numName=False):
        '''
        Return the first node that contains all of the tips in descendants.
        numName is a flag for whether the tip name is in the 'numName' field instead of 'name'.
        '''

        types=[desc.__class__ for desc in descendants] # Check all descendants are the same type
        assert len(set(types))==1,'More than one type of data detected in descendants list'
        if numName==True: # Check all descendant tips are in the tree
            assert sum([1 if k.numName in [w.numName for w in self.Objects if w.branchType=='leaf'] else 0 for k in descendants])==len(descendants),'Not all specified descendants are in tree: %s'%(descendants)
        else:
            assert sum([1 if k.name in [w.name for w in self.Objects if w.branchType=='leaf'] else 0 for k in descendants])==len(descendants),'Not all specified descendants are in tree: %s'%(descendants)

        dtype=list(set(types))[0]
        # Find all nodes with >= n descendants, where n is the # descendants in the passed list. ## Sort them from root --> tips
        allAncestors=sorted([k for k in self.Objects if k.branchType=='node' and len(k.leaves)>=len(descendants)],key=lambda x:x.height)

        possible_ancestors = [] # all nodes that contain all the descendants
        if numName==True:
            for k in allAncestors:
                k_contains = [l.numName in k.leaves for l in descendants]
                if all(k_contains):
                    possible_ancestors.append(k)
        else:
            for k in allAncestors:
                k_contains = [l.name in k.leaves for l in descendants]
                if all(k_contains):
                    possible_ancestors.append(k)

        return possible_ancestors[-1] ## Return the least basal node that still contains all the descendants.
    ...
    def collapse(self,cl,givenName,verbose=False):
        ...
        collapsedClade.tipHeight = self.traverse_tree(cl)[0].height
        collapsedClade.width = cl.numChildren
        ...
        collapsedClade.n = cl.numChildren
        if verbose==True:
        ...
        for k in self.traverse_tree(cl,include_all=True):
            remove_from_tree.append(k.index)

        for k in remove_from_tree:
            k = [i for i in self.Objects if i.index == k ][0]
