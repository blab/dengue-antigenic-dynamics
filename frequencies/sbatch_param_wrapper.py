...
for antigenic_resolution in antigenic_resolutions:
	for clade_resolution in clade_resolutions:
		...
		for (b,g,s) in product(beta_vals, gamma_vals, sigma_vals):
			...
			cmd = 'python ./antigenic_fitness.py --frequency_path %s --titer_path %s --out_path %s --name %s --years_back 2 --years_forward 5 --beta %f --gamma %f --sigma %f --DENV1_f0 1'%(frequency_path, titer_path, out_path, name, b,g,s)
